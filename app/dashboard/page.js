...
export default function Dashboard() {
  ...
  const handleUpload = async () => {
    ...
    await supabase.from('works').insert({ user_id: user.id, title, description, authors, category_id: categoryId, file_url: publicUrl, file_name: file.name })
    setTitle(''); setDescription(''); setAuthors(''); setFile(null); setShowForm(false)
    fetchWorks(user.id)
    setUploading(false)
  }
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        {showForm && (
          <div className="bg-white rounded-xl shadow p-6 mb-6">
            ...
            <input placeholder="Sarlavha *" value={title} onChange={e => setTitle(e.target.value)} className="w-full border rounded-lg p-3 mb-3 outline-none focus:border-blue-500" />
            ...
            <input placeholder="Mualliflar (masalan: A.Karimov, B.Rahimov)" value={authors} onChange={e => setAuthors(e.target.value)} className="w-full border rounded-lg p-3 mb-3 outline-none focus:border-blue-500" />
            ...
            <input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={e => setFile(e.target.files[0])} className="w-full border rounded-lg p-3 mb-4" />
